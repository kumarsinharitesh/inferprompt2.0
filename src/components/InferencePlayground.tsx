...
const InferencePlayground: React.FC = () => {
  ...
  const canRun = stream.status !== "streaming" && credits > 0 && (
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-4 sm:gap-6">
        <aside className="flex flex-col gap-4">
          ...
          {/* Credit Balance */}
          <div className="rounded-xl border border-[#1e1e2c] bg-[#12121a] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-2">Credits</p>
            <div className="flex items-center justify-between">
              <span className={`text-lg font-bold ${credits <= 0 ? "text-red-400" : credits <= 2 ? "text-amber-400" : "text-emerald-400"
                }`}>{credits} remaining</span>
              <button
                type="button"
                onClick={() => setShowCreditModal(true)}
                className="text-[11px] px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20
                             hover:bg-amber-500/20 transition-colors font-medium"
              >
                Buy Credits
              </button>
            </div>
            <p className="text-[10px] text-slate-600 mt-1.5">1 credit per 100 tokens · min 1/run</p>
            {credits <= 0 && (
              <p className="text-[11px] text-red-400 mt-2 font-medium">⚠ Out of credits — purchase more to continue.</p>
            )}
          </div>
        </aside>
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div role="tablist" aria-label="Input mode" className="flex gap-1 p-1 bg-[#12121a] border border-[#1e1e2c] rounded-xl w-fit">
            {(["text", "audio"] as Mode[]).map(m => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-amber-500/40 ${mode === m ? "bg-amber-500 text-black" : "text-slate-400 hover:text-slate-200"
                  }`}
              >
                {m === "text" ? "Text" : "Audio"}
              </button>
          ...
          <StreamingOutput
            ...
            error={stream.error}
            liveTokenCount={stream.liveTokenCount}
            onRetry={() => void submit()}
      ...
      {showCreditModal && (
        <CreditPurchaseModal
          ...
          onSuccess={async () => {
            ...
            await refreshUser(); // Reload server balance everywhere simultaneously
          }}
        />
      )}
    </>
